...
import { MapPin, Mail, Phone, ExternalLink, Quote, Star, Menu, X, User } from 'lucide-react';
...
const ModernTemplate: React.FC<ModernTemplateProps> = ({ data }) => {
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold text-gray-900">{data.hero.name}</span>
            </div>
              ...
              <div className="flex flex-col space-y-3">
                {navItems.map((item) => (
                  <button
                    key={item.id}
                    onClick={() => scrollToSection(item.id)}
                    className="text-left px-4 py-2 text-gray-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg font-medium transition-colors duration-200"
                  >
                    {item.label}
                  </button>
                ))}
      ...
      {/* Hero Section */}
      <section id="hero" className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-800 pt-16">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {data.about.socials.map((social, index) => (
                    <a
                      ...
                      className="flex items-center p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg hover:from-blue-100 hover:to-purple-100 transition-all duration-300"
                    >
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {data.skills.map((skill, index) => (
                <motion.span
                  ...
                  viewport={{ once: true }}
                  className="px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-full font-medium shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300"
                >
                  {skill}
                </motion.span>
              ))}
      ...
      <section id="portfolio" className="py-20 px-6 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-4xl font-bold text-gray-900 text-center mb-12">Portfolio</h2>
            <div className="grid md:grid-cols-3 gap-8">
      ...
      <section id="testimonials" className="py-20 px-6">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-4xl font-bold text-gray-900 text-center mb-12">What Clients Say</h2>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              ...
              <div className="bg-white p-8 rounded-2xl shadow-lg">
                <h3 className="text-2xl font-bold text-gray-900 mb-4">{data.blog.title}</h3>
                <p className="text-gray-600 leading-relaxed">{data.blog.summary}</p>
              </div>
            </motion.div>
          </div>
        </section>
      ...
      {/* Contact Section */}
      <section id="contact" className="py-20 px-6 bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-800 text-white">
        <div className="max-w-4xl mx-auto text-center">
          <motion.div
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
      ...
      <footer className="py-8 px-6 bg-gray-900 text-white text-center">
        <p>&copy; 2025 {data.hero.name}. All rights reserved.</p>
      </footer>
    </div>
  );
};
